...
# Modified version of: https://github.com/srggrs/assign-one-project-github-action
...
if [ "$PROJECT_TYPE" = org ] || [ "$PROJECT_TYPE" = user ]; then
  if [ -z "$CUSTOM_GITHUB_TOKEN" ]; then
    echo "CUSTOM_GITHUB_TOKEN not defined" >&2
    exit 1
  fi

  TOKEN="$CUSTOM_GITHUB_TOKEN" # It's User's personal access token. It should be secret.
else
...
INITIAL_COLUMN_NAME="$INPUT_COLUMN"
